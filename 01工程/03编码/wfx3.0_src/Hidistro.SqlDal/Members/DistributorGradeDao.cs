namespace Hidistro.SqlDal.Members
{
    ...
    public class DistributorGradeDao
    {
        ...
        public bool UpdateChannelList(Channel Channel)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE Hishop_ChannelList SET ChannelName=@ChannelName,Remark=@Remark WHERE Id=@Id");
            this.database.AddInParameter(sqlStringCommand, "Id", DbType.Guid, Channel.Id);
            this.database.AddInParameter(sqlStringCommand, "ChannelName", DbType.String, Channel.ChannelName);
            this.database.AddInParameter(sqlStringCommand, "Remark", DbType.String, Channel.Remark);    
            return (this.database.ExecuteNonQuery(sqlStringCommand) > 0);
        }

        public bool InsertChannelList(Channel query)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("Insert into Hishop_ChannelList(id,ChannelName,Remark) values (newid(),@ChannelName,@Remark)");
            this.database.AddInParameter(sqlStringCommand, "ChannelName", DbType.String, query.ChannelName);
            this.database.AddInParameter(sqlStringCommand, "Remark", DbType.String, query.Remark);
            return (this.database.ExecuteNonQuery(sqlStringCommand) > 0);
        }

        public DataTable GetChannelList()
        {
            DbCommand sqlCommand = this.database.GetSqlStringCommand("select id,ChannelName from Hishop_ChannelList");
            return this.database.ExecuteDataSet(sqlCommand).Tables[0];
        }
        ...
        public DbQueryResult GetChannelListGradeRequest(Channel query)
        {
            StringBuilder builder = new StringBuilder();
            if (!string.IsNullOrEmpty(query.ChannelName))
            {
                ...
                builder.AppendFormat(" ChannelName LIKE '%{0}%'", DataHelper.CleanSearchString(query.ChannelName));
            }
            return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "Hishop_ChannelList ", "Id", (builder.Length > 0) ? builder.ToString() : null, "*");
        }
