...
function RequestCommissions(obj) {
    ...
    if (obj == 2 && $("#VRequestCommissions_txtaccount").val().replace(/\s/g, "") == "") {
        alert_h('请输入银行卡号');
        return false;
    }
    var requesttype = obj;
    ...
    switch (requesttype) {
        case "1":
            account = "支付宝账号:" + $("#VRequestCommissions_txtaccount").val() + ",姓名:" + $("#VRequestCommissions_txtaccountname").val();
            break;
        case "0"://微信支付
        case "2": //银行支付
            account = "银行卡号:" + $("#VRequestCommissions_txtaccount").val() + ",户主:" + $("#VRequestCommissions_txtaccountname").val() + ",开户行:" + $("#VRequestCommissions_txtaccountbank").val();
            break;
    }
    var money = 0;
    if (requesttype == 0) { //微信支付
        account = "";
  ...
  || parseFloat($("#VRequestCommissions_txtmoneyweixin").val().replace(/\s/g, "")) <= 0
        ...
        //新增对姓名的非空判断
        if ($("#VRequestCommissions_txtaccountname").val().replace(/\s/g, "") == "") {
            alert_h('请输入您的姓名');
            return false;
        }

        if (requesttype == 2 && $("#VRequestCommissions_txtaccountbank").val().replace(/\s/g, "") == "") {
            alert_h('请输入您的开户行');
            return false;
        }
  ...
  || parseFloat($("#VRequestCommissions_txtmoney").val().replace(/\s/g, "")) <= 0
    ...
    switch ($("#requestType").val()) {
        case "0":
            ...
            break;
        case "1":
            str = "支付方式：支付宝提现\n支付宝账号：" + $("#VRequestCommissions_txtaccount").val() + "\n姓名：" + $("#VRequestCommissions_txtaccountname").val() + "\n提现金额：￥" + money;
            break;
        case "2":
            str = "支付方式：银行卡提现\n银行卡号：" + $("#VRequestCommissions_txtaccount").val() + "\n户主：" + $("#VRequestCommissions_txtaccountname").val() + "\n开户行：" + $("#VRequestCommissions_txtaccountbank").val() + "\n提现金额：￥" + money;
            break;
    }
    if (!confirm("确定以下提现信息无误吗?\r\n" + str))
        return false;
...
$(function(){
    ...
    $("div[name='WenBen']").each(function () {
        var a = 1;
        var h1 = $(this).height();
        var h2 = $(this).find(".wb_txt p").height();
        if (h1 < h2 && $(this).find('.moretxt').length == 0) {
            $("<div class='moretxt'><img src='/Templates/vshop/common/images/moretxt.png'></div>").appendTo(this);
            $(this).click(function () {
                if (a == 1) {
                    $(this).animate({ height: h2 });
                    $(this).find(".wb_txt").animate({ height: h2 });
                    $(this).find(".moretxt img").attr("src", "/Templates/vshop/common/images/moretxt1.png")
                    a = 2;
                } else {
                    $(this).animate({ height: h1 });
                    $(this).find(".wb_txt").animate({ height: h1 });
                    $(this).find(".moretxt img").attr("src", "/Templates/vshop/common/images/moretxt.png")
                    a = 1;
                }
            });  
        }
