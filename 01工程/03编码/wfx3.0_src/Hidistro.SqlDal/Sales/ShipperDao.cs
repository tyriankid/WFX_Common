namespace Hidistro.SqlDal.Sales
{
    ...
    public class ShipperDao
    {
        ...
        public ShippersInfo GetIsDefault(int isDefault)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM Hishop_Shippers WHERE isDefault = @isDefault");
            this.database.AddInParameter(sqlStringCommand, "isDefault", DbType.Int32, isDefault);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return ReaderConvert.ReaderToModel<ShippersInfo>(reader);
            }
