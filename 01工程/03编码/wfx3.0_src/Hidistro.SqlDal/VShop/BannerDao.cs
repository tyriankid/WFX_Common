namespace Hidistro.SqlDal.VShop
{
    ...
    public class BannerDao
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public bool DelImgCfg(int id)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(new StringBuilder(" delete from Hishop_ImgBanner where BannerId=@BannerId").ToString());
            this.database.AddInParameter(sqlStringCommand, "BannerId", DbType.Int32, id);
            return (this.database.ExecuteNonQuery(sqlStringCommand) > 0);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public IList<BannerInfo> GetAllImgBanners()
        {
            IList<BannerInfo> list = new List<BannerInfo>();
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(new StringBuilder("select * from  Hishop_ImgBanner where type=1 ORDER BY DisplaySequence ASC").ToString());
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return ReaderConvert.ReaderToList<BannerInfo>(reader);
            }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public TplCfgInfo GetImgCfgById(int id)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(new StringBuilder(" select * from Hishop_ImgBanner where BannerId=@BannerId").ToString());
            this.database.AddInParameter(sqlStringCommand, "BannerId", DbType.Int32, id);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return ReaderConvert.ReaderToModel<TplCfgInfo>(reader);
        ...
        /// <returns></returns>
        public bool SaveImgCfg(TplCfgInfo info)
        {
            int maxBannerSequence = this.GetMaxBannerSequence();
            StringBuilder builder = new StringBuilder("insert into  Hishop_ImgBanner (ShortDesc,ImageUrl,DisplaySequence,LocationType,Url,Type,IsDisable)");
            builder.Append("values (@ShortDesc,@ImageUrl,@DisplaySequence,@LocationType,@Url,@Type,@IsDisable)");
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
            this.database.AddInParameter(sqlStringCommand, "ShortDesc", DbType.String, info.ShortDesc);
            this.database.AddInParameter(sqlStringCommand, "ImageUrl", DbType.String, info.ImageUrl);
        ...
        /// <param name="info"></param>
        /// <returns></returns>
        public bool UpdateImgCfg(TplCfgInfo info)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("update Hishop_ImgBanner set ");
            builder.Append("ShortDesc=@ShortDesc,");
            builder.Append("ImageUrl=@ImageUrl,");
            builder.Append("DisplaySequence=@DisplaySequence,");
            builder.Append("LocationType=@LocationType,");
