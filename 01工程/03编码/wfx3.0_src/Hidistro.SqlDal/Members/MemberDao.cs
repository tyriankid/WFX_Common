namespace Hidistro.SqlDal.Members
{
    ...
    public class MemberDao
    {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO aspnet_Members(GradeId,ReferralUserId,UserName,CreateDate,OrderNumber,Expenditure,Points,TopRegionId, RegionId,OpenId, SessionId, SessionEndTime,Password,UserHead,RegSource,AliOpenId) VALUES(@GradeId,@ReferralUserId,@UserName,@CreateDate,0,0,0,0,0,@OpenId, @SessionId, @SessionEndTime,@Password,@UserHead,@RegSource,@AliOpenId)");
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE aspnet_Members SET GradeId = @GradeId,OpenId = @OpenId,UserName = @UserName, RealName = @RealName, TopRegionId = @TopRegionId, RegionId = @RegionId,VipCardNumber = @VipCardNumber, VipCardDate = @VipCardDate, Email = @Email, CellPhone = @CellPhone, QQ = @QQ, Address = @Address, Expenditure = @Expenditure, OrderNumber = @OrderNumber,MicroSignal=@MicroSignal,UserHead=@UserHead WHERE UserId = @UserId");
        ...
        /// 设置推荐用户id
        /// </summary>
        public bool SetFromUserId(int fromUserId,int currentMemberUserId)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("update aspnet_members set fromUserId = @fromUserId where UserId = @userid");
            this.database.AddInParameter(sqlStringCommand, "fromUserId", DbType.Int32, fromUserId);
            this.database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, currentMemberUserId);
            return (this.database.ExecuteNonQuery(sqlStringCommand) > 0);
        }

        public DataTable debugDT(string sql)
        {
        ...
        public int debugFuckIt(string sql)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(sql);
