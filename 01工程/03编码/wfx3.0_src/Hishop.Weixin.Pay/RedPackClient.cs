namespace Hishop.Weixin.Pay
{
    ...
    public class RedPackClient
    {
        ...
        public static string Send(string cert, string password, byte[] data, string url)
        {
            ...
            WriteLogEx("40_02:" + certificate);
            X509Certificate2 certificate2 = new X509Certificate2(cert, password, X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.MachineKeySet);
                WriteLogEx("40_certificate2:" );
            WriteLogEx("40_03:" );
            X509Store store = new X509Store(StoreName.My, StoreLocation.CurrentUser); WriteLogEx("40_store:" + store);
            try
            {
                store.Open(OpenFlags.ReadWrite); WriteLogEx("40_store.Open:" + store);
            }
            catch(Exception e)
            { WriteLogEx("40_e:"+e.Message); }
            store.Remove(certificate2);//certificate2
            store.Add(certificate2);//certificate2
            store.Close(); WriteLogEx("40_store.Close:" );
            HttpWebRequest request = WebRequest.Create(url) as HttpWebRequest;
            WriteLogEx("40_request:" + request);
            if (request == null)
            {
            ...
            request.Method = "POST";
            request.ContentLength = data.Length; WriteLogEx("40_data.Length:" + data.Length);
            ...
            {
                WriteLogEx("40_2:" + exception.Message);
                throw exception;
            }
        ...
        {
            string path = HttpContext.Current.Server.MapPath("/wx_login.txt");
