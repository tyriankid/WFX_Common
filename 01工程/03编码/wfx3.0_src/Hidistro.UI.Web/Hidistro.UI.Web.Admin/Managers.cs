...
namespace Hidistro.UI.Web.Admin
{
	...
	public class Managers : AdminPage
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            this.grdManager.RowDataBound += new System.Web.UI.WebControls.GridViewRowEventHandler(this.grdManager_RowDataBound);
        ...
        /// <param name="e"></param>
        private void grdManager_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
        {
            if (e.Row.RowType == System.Web.UI.WebControls.DataControlRowType.DataRow)
            {
                System.Web.UI.WebControls.Literal literal = (System.Web.UI.WebControls.Literal)e.Row.FindControl("lblRoleName");
                int result = 0;
                if(!string.IsNullOrEmpty(literal.Text) && int.TryParse(literal.Text,out result))
                {
                    IList<RoleInfo> roles = ManagerHelper.GetRoles();
                    foreach (RoleInfo info in roles)
                    {
                        if (info.RoleId.Equals(result))
                        {
                            literal.Text = info.RoleName;
                            break;
                        }
                    }
                }
