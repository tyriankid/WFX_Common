                        ...
                        <tbody>
                            <tr>
                                ...
                                <td width="180"><%# Eval("RequestTime", "{0:yyyy-MM-dd}")%>&nbsp;</td>
                                 ...
                                 <td width="200"><%# Eval("RequestType").ToString()=="1"?Eval("MerchantCode"):"微信红包"%>&nbsp;<!-- 针对同欣美容更改了收款方式内容 -->
                                </td>
                                <td width="60">
                                    <span class="submit_bianji"><a <%#Eval("RequestType").ToString()=="1"?" style='color:red;cursor:pointer;' onclick=\"Apply("+ Eval("SerialID")+","+Eval("Amount")+","+Eval("ReferralBlance")+",'"+Eval("MerchantCode")+"',"+Eval("UserId")+","+Eval("RedpackRecordNum")+")\"":" style='cursor:pointer' onclick=\"if(confirm('客户选择的是【微信红包】提现，您确定要选择线下打款操作吗？')){Apply("+ Eval("SerialID")+","+Eval("Amount")+","+Eval("ReferralBlance")+",'"+Eval("MerchantCode")+"',"+Eval("UserId")+","+Eval("RedpackRecordNum")+")}\""%>>
                                        线下打款</a></span>
                                    <span class="submit_bianji" style="display:none"><!-- 针对同欣美容取消了红包发放功能 -->
                                            <asp:LinkButton ID="lkBtnSendRedPack" runat="server" CommandName="sendredpack" CommandArgument='<%#Eval("SerialID") %>'>生成微信红包</asp:LinkButton>
