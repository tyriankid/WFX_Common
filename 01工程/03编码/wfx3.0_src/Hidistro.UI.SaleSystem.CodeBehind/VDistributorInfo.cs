        ...
        protected override void AttachChildControls()
        {
            ...
            if (userIdDistributors != null)
            {
                    ...
                    foreach (string str in userIdDistributors.BackImage.Split(new char[] { '|' }).ToList<string>())
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            if (!string.IsNullOrEmpty(str))
                            {
                                ...
                                this.litBackImg.Text = string.Concat(new object[] { text, " <div class=\"upFile clearfix\" style=\"float: left;\"><div class=\"bgImg\" ><img id=\"BakImg", num, "\" src=\"", str, "\" /></div><div class=\"adds\"><input id=\"BakFile", num, "\" name=\"BakFile", num, "\" type=\"file\" accept=\"image/*\" onchange=\"return FileUpload_onselect('BakImg", num, "')\"/></div></div>" });
                            }
                        }
                    }
                }
                while (num < 4)
                {
                    ...
                    this.litBackImg.Text = string.Concat(new object[] { obj3, "<div class=\"upFile clearfix\" style=\"float: left;\"><div class=\"bgImg\"><img id=\"BakImg", num, "\" src=\"http://fpoimg.com/100x100/product\" /></div><div class=\"adds\"><input id=\"BakFile", num, "\" name=\"BakFile", num, "\" type=\"file\" accept=\"image/*\" /></div></div>" });
                }
            }
        }
