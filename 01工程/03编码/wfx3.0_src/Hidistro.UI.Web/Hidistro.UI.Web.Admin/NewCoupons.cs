...
namespace Hidistro.UI.Web.Admin
{
    ...
    public class NewCoupons : AdminPage
	{
        ...
        public static string updateIsSend(string couponId,string type,string where)
        {
            switch (where)
            {
                case "home"://首页
                    if (CouponHelper.UpdateIsSendAtHomepage(Convert.ToInt32(couponId), type))
                    {
                ...
                case "dis"://成为分销商时
                    if (CouponHelper.UpdateIsSendAtDistributor(Convert.ToInt32(couponId), type))
                    {
                        return string.Format("success");
                    }
                    else
